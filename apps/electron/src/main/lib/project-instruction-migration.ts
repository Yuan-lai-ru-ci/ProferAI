...
/**
 * 生成 legacy CLAUDE.md → AGENTS.md 的用户主动迁移建议。
 *
 * 普通项目操作不应调用此 helper；迁移是用户显式发起的独立动作。
 *
...
export function buildLegacyProjectMigrationPrompt(options: LegacyMigrationPromptOptions): string | undefined {
  ...
  return `${heading} Legacy 项目指令迁移建议

Profer 已从受信任项目根加载以下 legacy \`CLAUDE.md\` 兼容来源；如果用户主动选择迁移，可将其提炼到**同目录**的 \`AGENTS.md\`：\n${legacySources.map(formatSourceEntry).join('\n')}${activeAgentsSection}

仅在用户明确发起迁移时，逐个完成以下证据驱动的迁移：
...
4. **安全写入与报告**：只创建或最小增量修改目标 \`AGENTS.md\`，不整体覆盖已有内容；保留用户原有 \`CLAUDE.md\`，不得重命名或删除。完成后简短报告：读取的证据、保留/舍弃或待确认的规则、以及写入的 scope。
