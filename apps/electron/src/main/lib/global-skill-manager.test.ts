...
import { cpSync, existsSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, renameSync, rmSync, utimesSync, writeFileSync } from 'node:fs'
...
import { createWorkspaceSkill, getWorkspaceSkills, toggleWorkspaceSkill } from './agent-workspace-manager'
...
`)
  ...
  test('Given 全局 slug 仅大小写或 Unicode 组合形式不同 When 创建 Then 按 Windows 唯一键拒绝', () => {
    setup()
    expect(() => createUserGlobalSkill('DEMO', '大小写重复', '', '# 重复')).toThrow('slug 已存在')
    createUserGlobalSkill('CAFÉ', '组合字符', '', '# 组合字符')

    expect(() => createUserGlobalSkill('cafe\u0301', '重复组合字符', '', '# 重复')).toThrow('slug 已存在')
  })

  test('Given 历史全局定义存在 canonical 冲突 When 解析 runtime Then 仅投影一个目录', () => {
    const paths = setup()
    const duplicateId = 'legacy-case-duplicate'
    const duplicate = join(paths.global, 'user', duplicateId)
    mkdirSync(duplicate, { recursive: true })
    writeFileSync(join(duplicate, 'SKILL.md'), '---\nname: 历史重复\nversion: 1.0.0\n---\n\n# 历史重复\n')
    writeFileSync(join(duplicate, 'skill.manifest.json'), JSON.stringify({
      schemaVersion: 1,
      skillId: duplicateId,
      slug: 'DEMO',
      type: 'user-global',
    ...
    const projection = prepareRuntimeSkills('legacy-global-conflict')
    expect(projection.skills).toHaveLength(1)
    expect(projection.skills[0]?.slug).toBe('demo')
    expect(readdirSync(join(projection.path, 'skills'))).toEqual(['demo'])
  })

  test('Given 工作区已有 canonical 同名 Skill When 创建大小写变体或 Windows 非法名称 Then 后端拒绝', () => {
    const configRoot = mkdtempSync(join(tmpdir(), 'profer-workspace-skill-portable-'))
    roots.push(configRoot)
    const previousConfigRoot = process.env.PROFER_CONFIG_DIR
    process.env.PROFER_CONFIG_DIR = configRoot
    try {
      createWorkspaceSkill('portable', 'CAFÉ', '组合字符', '', '# 组合字符')
      expect(() => createWorkspaceSkill('portable', 'cafe\u0301', '重复', '', '# 重复')).toThrow('Skill 已存在')
      expect(() => createWorkspaceSkill('portable', 'CON', '设备名', '', '# 非法')).toThrow('Windows 保留设备名')
      expect(() => createWorkspaceSkill('portable', 'bad.', '尾点', '', '# 非法')).toThrow('不能以点或空格结尾')
      expect(() => createWorkspaceSkill('portable', 'bad ', '尾空格', '', '# 非法')).toThrow('不能以点或空格结尾')
    } finally {
  ...
  test('Given 工作区与全局 slug 仅大小写不同 When 解析 runtime Then 工作区优先且只投影一次', () => {
    const paths = setup()
    const local = join(paths.workspaces, 'case-conflict', 'skills', 'DEMO')
    mkdirSync(local, { recursive: true })
    writeFileSync(join(local, 'SKILL.md'), '---\nname: 本地演示\nversion: 1.0.0\n---\n\n# 本地演示\n')
    writeFileSync(join(local, '.source.json'), JSON.stringify({ scope: 'workspace', workspaceSkillId: 'case-conflict-local' }))
    const builtin = listGlobalSkills()[0]!
    expect(() => copyGlobalSkillToWorkspace(builtin.skillId, 'case-conflict')).toThrow('已存在同名 Skill')

    const projection = prepareRuntimeSkills('case-conflict')
    expect(projection.skills).toHaveLength(1)
    expect(projection.skills[0]).toEqual(expect.objectContaining({ slug: 'DEMO', scope: 'workspace' }))
    expect(projection.diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'workspace-slug-conflict', slug: 'demo' }),
    ]))
    expect(existsSync(join(projection.path, 'skills', 'DEMO', 'SKILL.md'))).toBe(true)
    expect(readdirSync(join(projection.path, 'skills'))).toEqual(['DEMO'])
  })
...
# 仅工作区`)
  ...
  test('工作区复制替换只影响当前 workspace 且不重复解析', () => {
    ...
    setGlobalSkillEnabled('c', builtin.skillId, true)
    expect(resolveEffectiveSkills('c').filter((s) => s.slug === 'demo')).toHaveLength(1)
  })
  ...
  test('副本删除后只有显式恢复才重新加载原全局 Skill', () => {
    ...
    // 删除副本后，读取 override 会自动清理失效 replacement 并恢复全局来源。
    expect(resolveEffectiveSkills('a').filter((skill) => skill.slug === 'demo')).toHaveLength(1)
    restoreGlobalSkill('a', builtin.skillId)
    expect(resolveEffectiveSkills('a').filter((skill) => skill.slug === 'demo')).toHaveLength(1)
    expect(resolveEffectiveSkills('a').find((skill) => skill.slug === 'demo')?.scope).toBe('global')
