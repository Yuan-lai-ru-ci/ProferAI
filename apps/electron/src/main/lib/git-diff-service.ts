...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@profer/shared'
import { normalizePathForCompare } from '@profer/shared'
import type { ChangeSource, ChangedFileStatus } from '@profer/shared'
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@profer/shared').WorktreeInfo[]> {
  ...
  const worktrees: import('@profer/shared').WorktreeInfo[] = []
...
export async function getWorktreeChanges(
...
): Promise<import('@profer/shared').UnstagedChangesResult> {
  ...
  const allFiles: import('@profer/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@profer/shared').ChangedFileEntry>()
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@profer/shared').ChangedFileStatus
      let filePath: string
      ...
      const entry: import('@profer/shared').ChangedFileEntry = {
        filePath,
        status,
        additions: stats.additions,
        deletions: stats.deletions,
  ...
  // 3. 新文件（未追踪）
  const untrackedFiles: import('@profer/shared').UntrackedFileEntry[] = []
  const untrackedOutput = await runGitCommand(['ls-files', '--others', '--exclude-standard'], gitRoot)
  if (untrackedOutput) {
    for (const rel of untrackedOutput.split('\n').filter(Boolean)) {
      if (!fileMap.has(rel)) {
