...
import { PLANNING_CONFLICT_ERROR, AUTOMATION_OCCURRENCE_SAMPLES_PER_DAY, getAutomationOccurrencesByDay } from '@profer/shared'
...
import { PlanningTagManager } from '@/components/planning/PlanningTagManager'
...
export function CalendarWorkspace(): React.ReactElement {
  ...
  const [tags, setTags] = useAtom(planningTagsAtom)
  ...
  const tagUsageCounts = React.useMemo(() => {
    const counts = new Map<string, number>()
    for (const event of events) for (const tag of event.tags) counts.set(tag.id, (counts.get(tag.id) ?? 0) + 1)
    for (const todo of todos) for (const tag of todo.tags) counts.set(tag.id, (counts.get(tag.id) ?? 0) + 1)
    return counts
  }, [events, todos])
  const createPlanningTag = React.useCallback(async (name: string): Promise<PlanningTag | undefined> => {
    try {
      ...
      console.error('[日程] 创建标签失败:', error)
      toast.error('创建标签失败：名称可能已存在')
      return undefined
    }
  }, [setTags])
  const renamePlanningTag = React.useCallback(async (tag: PlanningTag, name: string): Promise<PlanningTag | undefined> => {
    try {
      ...
      console.error('[日程] 重命名标签失败:', error)
      toast.error('重命名标签失败：名称可能已存在')
      return undefined
    }
  }, [setEvents, setTags])
...
function formatAutomationSchedule(automation: Automation): string {
  const time = (automation.timeOfDay?.length ? automation.timeOfDay.join('/') : '09:00')
  ...
  if (automation.scheduleType === 'weekly') {
    ...
    const days = (automation.dayOfWeek?.length ? automation.dayOfWeek : [1]).map((day) => weekdays[day] ?? `星期${day}`)
    return `每${days.join('、')} ${time}`
  }
  if (automation.scheduleType === 'monthly') {
    const days = (automation.dayOfMonth?.length ? automation.dayOfMonth : [1]).join('、')
