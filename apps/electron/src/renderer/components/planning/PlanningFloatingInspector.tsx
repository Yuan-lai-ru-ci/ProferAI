...
export function PlanningFloatingInspector({ label, onClose, children }: PlanningFloatingInspectorProps): React.ReactElement {
  ...
  return (
    <>
      <div aria-hidden className="absolute inset-0 z-30 bg-foreground/[0.035] backdrop-blur-[1px]" onMouseDown={onClose} />
      <aside role="dialog" aria-label={label} className="absolute bottom-3 right-3 top-3 z-40 w-[min(30rem,calc(100%-1.5rem))] overflow-y-auto rounded-2xl border border-border/50 bg-card shadow-[0_18px_45px_rgb(0_0_0_/_0.14)] scrollbar-thin" onMouseDown={(event) => event.stopPropagation()}>
