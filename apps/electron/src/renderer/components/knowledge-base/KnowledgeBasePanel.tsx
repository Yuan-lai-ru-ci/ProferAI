...
export function KnowledgeBasePanel() {
  ...
  const retrySelectedPaperSync = useCallback(async () => {
    if (!selected?.legacy) return
    const paper = selected.meta as PaperMeta
    if (paper.source !== 'local') return
    if (paper.syncState !== 'failed' && paper.syncState !== 'pending') return
    try {
      await window.electronAPI.kb.retryPaperSync(paper.id)
      toast.success('论文已同步到 Paperpipe')
      await load(true)
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : '论文重新同步失败')
      await load(true)
    }
  }, [load, selected])
