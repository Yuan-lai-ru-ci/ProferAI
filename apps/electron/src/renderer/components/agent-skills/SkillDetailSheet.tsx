...
function SkillDetailBody({
  ...
  React.useEffect(() => {
    ...
    window.electronAPI
      .readSkillContent(workspaceSlug, skill.slug)
  ...
  const saveMeta = async (): Promise<void> => {
    ...
    setSaving(true);
    try {
      const newContent = rebuildSkillMd(content, {
        name: editName,
        description: editDescription,
      });
      await window.electronAPI.writeSkillContent(
        workspaceSlug,
        skill.slug,
        newContent,
      );
          ...
          <div className="flex flex-col gap-4 p-5">
            ...
            <div className="space-y-2">
              ...
              <SettingsCard divided>
                ...
                <MetaRow
                  label="来源 ID"
                  value={skill.sourceSkillId ?? "无（工作区自有 Skill）"}
                />
                <MetaRow
                  label="来源类型"
                  value={skill.sourceSkillType ?? "workspace-local"}
                />
                <MetaRow label="来源版本" value={skill.sourceVersion ?? "无"} />
                <MetaRow label="复制时间" value={skill.copiedAt ?? "无"} />
                <MetaRow
                  label="替换来源"
                  value={skill.replacementForSkillId ?? "无"}
                />
                <MetaRow
                  label="来源状态"
                  value={
                    skill.sourceStatus === "unknown-legacy"
                      ? "来源无法确认"
                      : skill.sourceStatus === "deleted"
                        ? "来源已删除"
                        : skill.sourceStatus === "available"
                          ? "可用"
                          : "无来源"
                  }
                />
