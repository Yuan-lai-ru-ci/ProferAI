...
interface OnboardingViewProps {
  ...
  initialStep?: 'intro' | 'welcome' | 'featureTour' | 'environment' | 'migration'
...
export function OnboardingView({
  ...
  const [step, setStep] = useState<'intro' | 'welcome' | 'featureTour' | 'environment' | 'migration'>(initialStep)
  ...
  const handleNextFromFeatureTour = () => {
    if (isWindows) {
      ...
      // 非 Windows 平台直接进入最后的迁移指引
      setStep('migration')
    }
  }

  const handleNextFromEnvironment = () => {
    setStep('migration')
  }
  ...
  return (
    <div className={cn(
      ...
      {step === 'featureTour' && (
        <FeatureTour
          finishLabel={isWindows ? '下一步：环境检测' : '下一步：导入数据'}
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <Button
                onClick={handleNextFromEnvironment}
                variant={shellOk ? 'default' : 'outline'}
              >
                {shellOk ? '下一步：导入数据' : '跳过环境检测，继续'}
              </Button>
            </div>
          </div>
        </div>
