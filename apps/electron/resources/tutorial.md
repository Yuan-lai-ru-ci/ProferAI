...
> 原教程：Erlich Liu（Proma 教程 v2，2026-06-11，v0.12.0）  
> 更新补充：ylrc（2026-06-21，v0.12.52）

---
## 写在开始之前（推荐看）

> 以下为 Erlich Liu 于 2026 年 6 月 11 日撰写的原版教程（v0.12.0），完整保留。
...
我和 Erlich Liu 是在一次成都开发者的线下聚会认识的。那时候我还是个迷茫的学生，但他已经是一个小有名气的「AI 游民」了。在我还不知 LangChain、Agent 为何物的时候，Erlich Liu 已经开始制作 Proma 了。我只能经常在朋友圈看到他的开发日常，心向往之，却苦于眼前的学业压力而无所作为。

我开始接触 Proma 是在今年 4 月。当时各种 Agent 概念炒得火热，我却兴致缺缺——用过 Codex 和 Claude Code 之类的 Agent 助手之后，我知道它们的能力还是十分有限的，只停留在表面，满足不了我稍微深入一些的需求。就在那时候，我遇到了 Proma。老实说，它是一个把很多好想法拼到一起的作品，但恰巧，它的很多需求和我不谋而合：美观整洁的界面、Agent 和 Chat 分离的对话方式、可以对接微信……这无疑是我的 all my need。

正因为它太好用了，我想让它和我、还有我的朋友们一起工作。于是我着手开始制作 Profer——基于 Proma 的小组协作 Agent，知识库共享、文件共享，同时每个人保留自己的工作区。这很像 Notion，我确实也是照着这个思路做的。

起初觉得这些功能不难实现，真正上手才知道和那些用 AI 做商业项目的大佬之间的差距。API 鉴权、密码加密、服务器握手——这些都是隐藏在表面之下的惊涛骇浪。我踩了很多坑，但也学会了更多。希望各位 Agent 时代的人才能与我一起无限进步！

下面是 Profer 的安装下载教程。所以我保留了 Erlich Liu 的原版教程——大家卡住的时候可以去看看原作者的深刻理解。

如果你是第一次接触 Profer，这部分带你从零开始走完：下载 → 注册 → 配置 → 开始使用。整个过程大约 10–15 分钟。
...
首次启动会自动环境检查。Windows 用户会被引导安装 Git、Node.js 等依赖，跟随界面操作即可。

### 2. 注册与登录（团队版）

Profer 同时支持**个人使用**和**团队协作**。个人使用时所有数据存本地，无需注册，直接跳到"配置 AI 渠道"。
...
- **Chat**：问答、翻译、润色、轻量代码讨论。输入即得回复。
- **Agent**：读写文件、执行命令、多步骤工作流。Agent 会主动使用工具并在执行前征求确认。
...
3. 观察 Agent 如何规划、征求确认、执行、输出结果。

右侧面板显示 Agent 操作的文件，左下角显示当前可用的 Skills 和 MCP 工具。
...
- 阅读下方 Erlich 的完整说明书——涵盖了文件组织、Skills 与 MCP、自动任务、远程访问等深度内容，是理解 Profer 设计哲学的关键
- 在 Agent 模式下输入 `/coach`，让 Profer Coach 帮你解决使用中的困惑
- 当你发现某类任务反复做时，让 Agent 帮你沉淀为 Skills（对话里说"帮我把刚才的工作流创建为 Skill"）
- 有飞书的话，在设置里配置飞书桥接，手机上也能用 Agent
...
最后，感谢你的阅读，感谢你关注 Proma，如果你有任何想法，欢迎跟我聊（微信：CYBER_YLRC），也欢迎为 Proma 提供 PR。
