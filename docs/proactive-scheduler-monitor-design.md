...
Proma 现在已经具备 Agent 会话、工作区 Skills/MCP、本地 JSON/JSONL 会话持久化、Quick Task、后台 Agent 执行、记忆工具、飞书通知、团队文件管理和本地持久化 Automation 等底座。下一步的关键不是再做一个孤立的 Cron 配置页，而是把现有 Automation 收束进 Proactive Center，让 Proma 从“用户发起一次对话”推进到“用户可以委托 Proma 长期关注、定期整理、持续跟进”的主动协作系统。
...
Claude 内置 Cron/Monitor 可以兼容和展示，但不应成为 Proma 的长期调度核心。Proma 已经有自己的 durable Automation scheduler，下一步应在此之上补齐 Monitor 与 Proactive Center，并把 `Proma_Proactive` 产品化为第一个官方级 proactive plugin/routine 样板。
...
1. 继续沿用 Proma 现有持久化 Automation scheduler，而不是依赖 Claude session-scoped Cron，也不另起一套平行调度系统。
2. 以 `Automation` / `AutomationRun` 作为第一阶段的 schedule/run 模型，后续再扩展 monitor/manual/recommendation 来源。
3. 支持 plugin/routine 声明，让 `Proma_Proactive` 这类外置能力可被 Proma 安装、配置、调度、审计。
4. 用 in-process MCP tools 暴露 Proma 的 Automation/Monitor/Memory 能力给 Agent。
...
一次实际运行记录。第一阶段复用当前 `Automation.runHistory` 中的 `AutomationRun`，记录触发时间、子会话、状态、耗时和错误信息。后续如果 Monitor、手动运行、Agent 创建等来源需要统一跨来源审计，可以在兼容 `AutomationRun` 的基础上扩展为更通用的 run record。
...
flowchart LR
  ...
  G --> H["AutomationRun / Approval / Memory"]
  H --> A
```
...
- Session 创建和 JSONL 写入由 Proma 现有 Agent session / headless run 层负责。
- SDK 调用由现有 `runAgentHeadless()` / orchestrator 负责。
- 权限由 Proma permission profile 负责。
- retry/resume 由现有 `automation-scheduler` / 后续 `ProactiveRunService` 负责。
...
sequenceDiagram
  ...
  participant R as ProactiveRunService
...
- 复用现有 `automation-scheduler.ts` 和 `automation-manager.ts`，加载 `~/.proma/automations.json`。
- 计算 `nextRunAt`。
- App 启动时恢复状态。
- 到点启动 headless Agent run 并追加 `AutomationRun`。
- 管理 missed run 策略。
- 管理并发和重试。
...
- 把事件转成 monitor run record；第一阶段可先复用 Automation 的 run 展示语义，后续再抽象统一 run 表。
...
- 调用 `runAgentHeadless()`。
- 捕获完成、错误、cost、turns。
- 链接 `AutomationRun` / monitor run 和 Proma session。
...
第一阶段以现有 `packages/shared/src/types/automation.ts` 为准：

- `Automation`：持久化任务定义，覆盖名称、prompt、调度类型、渠道、模型、工作区、权限模式、会话模式、通知目标、`nextRunAt`、`runHistory` 等。
- `AutomationRun`：单次运行记录，覆盖 `runAt`、`sessionId`、`status`、`durationMs`、`error`、`skipReason`。
- `AutomationScheduleType`：已支持 `interval` / `daily` / `weekly` / `monthly`。

下面的 `ProactiveScheduledTask` / `ProactiveRun` 是后续抽象层草案，用于把 Monitor、Recommendation、manual run 等来源统一到 Proactive Center。它们不应在第一阶段绕开现有 Automation 重新实现。

#### ProactiveScheduledTask

```ts
interface ProactiveScheduledTask {
  id: string
  automationId?: string
  title: string
  routineId: string
...
```text
~/.proma/
  automations.json                 # 已实现：Automation 配置 + runHistory
  proactive/
    recommendations.json
    approvals.json
    monitors.json
    runs/                          # 后续：跨 monitor/manual/agent 的统一 run 索引
      {runId}.json
      {runId}.jsonl
    plugins/
      proma-memory/
...
6. 用户确认。
7. Automation 创建 schedule，并写入 `~/.proma/automations.json`。
...
5. 状态变化时创建 monitor run record；第一阶段可按 Automation run 的展示语义进入 Runs。
6. Agent 分析变化并输出结果。
...
Scenario: Persist schedule across app restart
  ...
  When Proma restarts
  Then AutomationScheduler restores the schedule
...
Scenario: Trigger a monitor run
  ...
  Then MonitorService creates a monitor run record
...
Scenario: Run daily memory schedule
  ...
  And the run creates a visible AutomationRun record
...
Scenario: Stop a runaway proactive run
  Given a proactive run exceeds its max turns or cost budget
  When the proactive run layer detects the limit
...
- 将现有 Automation 入口升级 / 收束为 `Proactive Center / Today` 基础 UI。
- 复用现有 `AutomationsListView` / `AutomationFormView` 作为 `Schedules` 列表和详情的第一版。
- 复用 `automation-manager.ts` / `automation-scheduler.ts`，不另建平行 Scheduler。
- 复用 `Automation.runHistory` 作为第一版 Runs 数据源。
- 增加 `Monitor` 工具 UI 映射。
- 兼容展示 Claude 内置 `Cron*` 工具调用。

验收：

- 用户可以创建一个本地 durable Automation。
- App 重启后 Automation 仍存在并恢复 `nextRunAt`。
- 到点能创建 `AutomationRun` 记录，并能跳转到执行子会话。
...
建议下一步直接进入 Phase 1 + Phase 2 的设计实现，但以现有 Automation 为底座：

1. 先补齐 UI 层的 `Monitor` 工具映射和 Cron/Monitor 展示一致性。
2. 新增 `Proactive Center` 的 Today/Schedules/Runs 静态页面骨架，并把当前 Automation 列表接入 Schedules。
3. 将 `Automation.runHistory` 作为第一版 Runs 数据源，补足运行详情、失败原因和会话跳转。
