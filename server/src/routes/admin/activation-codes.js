...
// POST /v1/admin/activation-codes — 生成激活码（可指定订阅等级）
adminActivationCodes.post('/', async (c) => {
  const body = await c.req.json()
  const { count = 1, expiresInDays, membershipTier = 'free' } = body || {}
  ...
  for (let i = 0; i < Math.min(count, 100); i++) {
    ...
    const result = createActivationCode({ code, createdBy: c.get('userId'), expiresAt, membershipTier })
    codes.push(result)
  }

  logAudit({ action: 'admin.create_activation_codes', userId: c.get('userId'), userEmail: c.get('userEmail'), detail: `count=${codes.length} tier=${membershipTier}` })
  return c.json({ codes, membershipTier }, 201)
})
